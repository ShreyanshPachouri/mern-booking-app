    ...
    const handleStarsChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedStars((prevStars) =>
          event.target.checked
            ? [...prevStars, starRating]//If the current star, whose value is stored in the starRating variable is checked, add it into the stars array along with the previous stars.
            : prevStars.filter((star) => star !== starRating)//If the star is unchecked, remove it by filtering.
        );
      };
    ...
    return(
        <div className="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-5">
            ...
            <div className="flex justify-between items-center">
                <span className="text-xl font-bold">{hotelData?.pagination.total} Hotels found {search.destination? `in ${search.destination}` : ""}</span>
